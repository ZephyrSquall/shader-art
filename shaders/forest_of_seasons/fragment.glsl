...
// This piece was heavily rushed due to an impending deadline, so code was heavily re-used from the
// Stormy Flight fragment shader, hence many perspective-related things refer to lightning even
// though there is no lightning in this image. When I return to this piece to try to optimize it, I
// will also rename things so they actually refer to the parts of the image they affect.
...
const float CLOUD_SCROLL_SPEED = 16.0;
const float TREE_HORIZON = -0.505;
vec2 get_cloud_uv(vec2 uv) {
    // Apply perspective to clouds
    vec2 cloud_uv = uv;
    cloud_uv.y = 16.0 / (uv.y - TREE_HORIZON);
    cloud_uv.x *= cloud_uv.y * 0.8;

    if (uv.y > TREE_HORIZON) {
...
vec2 unget_cloud_uv(vec2 cloud_uv) {
    ...
    cloud_uv.x /= cloud_uv.y * 0.8;
    uv.y = (16.0 / cloud_uv.y) + TREE_HORIZON;
    uv.x = cloud_uv.x;

    return uv;
}

vec4 seasonal_color(vec4 spring_color, vec4 summer_color, vec4 autumn_color, vec4 winter_color) {
    float season_time = mod(u_time * 5.0, 60.0);
    float summer_proportion = smoothstep(10.0, 15.0, season_time);
    float autumn_proportion = smoothstep(25.0, 30.0, season_time);
    float winter_proportion = smoothstep(40.0, 45.0, season_time);
    float spring_proportion = smoothstep(55.0, 60.0, season_time);

    return mix(mix(mix(mix(spring_color, summer_color, summer_proportion), autumn_color, autumn_proportion), winter_color, winter_proportion), spring_color, spring_proportion);
}

vec4 lightning_bolt(vec2 uv, vec2 lightning_root_bolt, float seed, float scale) {
    vec4 color = vec4(0.0);
    ...
    vec2 image_uv = uv - lightning_root_bolt;
    image_uv.x /= -1.0;
    image_uv.y /= -3.0;

    color = composite(color, total_circle_color);

    if (0.0 <= image_uv.x && image_uv.x <= 1.0 && 0.0 <= image_uv.y && image_uv.y <= 1.0) {
        vec4 summer_tree = texture(u_tex2, image_uv);
        vec4 autumn_tree = texture(u_tex3, image_uv);
        vec4 winter_tree = texture(u_tex4, image_uv);
        vec4 spring_tree = texture(u_tex5, image_uv);
        vec4 tree_color = seasonal_color(summer_tree, autumn_tree, winter_tree, spring_tree);
        color = composite(tree_color, color);
    }
...
const float LIGHTNING_RESET_TIME = 35.0;

struct TreeWithDepth {
    vec4 tree_color;
    float depth;
};

TreeWithDepth generateLightning(vec2 uv, float initial_seed) {
    ...
    vec2 lightning_root_screen = vec2(
        random(seed + 0.01) * 0.1 + 2.0,
        (1.0 - pow(random(seed + 0.02), 3.0)) * 1.0 - 1.6
    );
    ...
    float scale = unget_cloud_uv(vec2(1.0 - CLOUD_SCROLL_SPEED * u_time, lightning_root_cloud.y)).x * 30.0;

    vec4 lightning_bolt = lightning_bolt(uv, lightning_root_bolt, seed, scale);

    TreeWithDepth tree_with_depth = TreeWithDepth(
        lightning_bolt, lightning_root_cloud.y
    );

    return tree_with_depth;
}

const float MOUNTAIN_SCROLL_SPEED = 5.0;
...
void main (void) {
    ...
    vec4 spring_color = vec4(0.28, 0.56, 0.41, 1.0);
    vec4 summer_color = vec4(0.4, 0.54, 0.22, 1.0);
    vec4 autumn_color = vec4(0.79, 0.41, 0.23, 1.0);
    vec4 winter_color = vec4(0.82, 0.88, 0.95, 1.0);

    vec4 sky_base = vec4(0.59, 0.81, 0.89, 1.0);
    vec4 sky_top = vec4(0.32, 0.42, 0.93, 1.0);

    vec2 cloud_uv = get_cloud_uv(uv);

    vec4 color = mix(mix(
        seasonal_color(summer_color, autumn_color, winter_color, spring_color),
        sky_base, smoothstep(-0.5, -0.4, uv.y)
    ), sky_top, smoothstep(-0.4, 1.0, uv.y));
    ...
    TreeWithDepth trees[TREES];
    for (int i = 0; i < TREES; i++) {
        trees[i] = generateLightning(uv, float(i));
    }
    ...
    while(!sorted) {
        ...
        for (int i = 0; i < TREES - 1; i++) {
            if (trees[i].depth > trees[i+1].depth) {
                TreeWithDepth temp = trees[i+1];
                trees[i+1] = trees[i];
                trees[i] = temp;
                sorted = false;
            }
    ...
    for (int i = 0; i < TREES; i++) {
        color = composite(trees[i].tree_color, color);
    }

    vec2 sorei_uv = image_uv * 1.0;
    sorei_uv.x = (sorei_uv.x * 1.15) - 0.05 + 0.05 * sin(u_time * 0.5);
    sorei_uv.y = (sorei_uv.y * 2.0) - 0.7 + 0.05 * cos(u_time * 2.0);
    if (sorei_uv.x > 0.0 && sorei_uv.x < 1.0 && sorei_uv.y > 0.0 && sorei_uv.y < 1.0) {
        vec4 sorei_winter = texture(u_tex1, sorei_uv);
        vec4 sorei_other = texture(u_tex0, sorei_uv);
        vec4 sorei_color = seasonal_color(sorei_other, sorei_other, sorei_winter, sorei_other);

        color = composite(sorei_color, color);
    }
    ...
    vec4 watermark_color = texture(u_tex6, watermark_uv);
    if (watermark_uv.x > 1.0 || watermark_uv.y > 1.0) {
        watermark_color = vec4(0);
    }
    watermark_color.a *= 0.2;
